        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {events.slice(0,30).map(e => (
              <tr key={e.id} className="border-b border-border/50"><td className="py-2 font-medium">{e.title}</td><td className="text-muted-foreground">{EVENT_TYPE_LABELS[e.event_type] ?? e.event_type}</td><td className="text-muted-foreground">{format(parseISO(e.start_time),"MMM d, p")}</td><td className="text-muted-foreground">{e.location ?? "—"}</td></tr>
            ))}
